...
abstract class Avram::Model
  ...
  macro table(table_name = nil)
    ...
    setup(Avram::Model.setup_table_name, {{ table_name }}, true)
    setup(Avram::Model.setup_initialize, {{ table_name }}, true)
    setup(Avram::Model.setup_db_mapping, {{ table_name }}, true)
    setup(Avram::Model.setup_getters, {{ table_name }}, true)
    setup(Avram::Model.setup_equals, {{ table_name }}, true)
    setup(Avram::Model.setup_column_names_method, {{ table_name }}, true)
    setup(Avram::BaseQueryTemplate.setup, {{ table_name }}, true)
    setup(Avram::BaseQueryTemplate.setup_with_primary_key, {{ table_name }}, false)
    setup(Avram::SaveOperationTemplate.setup, {{ table_name }}, false)
    setup(Avram::SchemaEnforcer.setup, {{ table_name }}, true)
  end
  ...
  macro setup(step, table_name, run_without_primary_key)
    {% has_primary_key = @type.has_constant?("PRIMARY_KEY_NAME") %}
    {% if has_primary_key || run_without_primary_key %}
      {{ step.id }}(
        type: {{ @type }},
        table_name: {{ table_name.id }},
        {% if has_primary_key %}primary_key_type: {{ PRIMARY_KEY_TYPE }},{% end %}
        {% if has_primary_key %}primary_key_name: {{ PRIMARY_KEY_NAME }},{% end %}
        columns: {{ COLUMNS }},
  ...
  macro setup_equals(*args, **named_args)
    {% if @type.has_constant?("PRIMARY_KEY_NAME") %}
      def_equals id, model_name
    {% else %}
      def_equals model_name
    {% end %}
  end
